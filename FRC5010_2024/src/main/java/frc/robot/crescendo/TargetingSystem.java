...
public class TargetingSystem extends GenericSubsystem {
	...
	public TargetingSystem(Supplier<Pose3d> targetSupplier, Supplier<Pose3d> robotPose, SwerveDrivetrain swerve,
			VisibleTargetSystem shooter, FeederSubsystem feeder, GenericGyro gyro) {
	...
	public Optional<Double> getShooterCamAngle() {
		if (useShooterCamera && shooterCamera.hasValidTarget()) {
			return Optional.of(interpolatePivotAngle(shooterCamera.getDistanceToTarget()));
		}
	...
	public Optional<Double> getShooterYawPower() {
		...
		if (useShooterCamera && shooterCamera.hasValidTarget()) {
			double angle = Units.degreesToRadians(shooterCamera.getTargetYaw() + SHOOTER_ANGLE_OFFSET);
	...
	public void periodic() {
		feeder.setShooterHasTarget(shooterCamera.hasValidTarget());
