...
public class SystemIdentification {
    ...
    private static final MutableMeasure<Velocity<Angle>> m_velocity = mutable(RotationsPerSecond.of(0));

    private static final MutableMeasure<Angle> m_angle_position = mutable(Rotations.of(0));
    private static final MutableMeasure<Velocity<Angle>> m_anglular_velocity = mutable(RotationsPerSecond.of(0));
    ...
    public static SysIdRoutine rpmSysIdRoutine(MotorController5010 motor, GenericEncoder encoder, String motorName, SubsystemBase subsystemBase) {
     ...
     public static SysIdRoutine angleSysIdRoutine(MotorController5010 motor, GenericEncoder encoder, String motorName, SubsystemBase subsystemBase) {
        ...
        return new SysIdRoutine(new Config(), new SysIdRoutine.Mechanism(
            ...
            log.motor(motorName)
                ...
                .angularVelocity(m_anglular_velocity.mut_replace(encoder.getVelocity(), DegreesPerSecond));
            }, subsystemBase));
     }

     public static Command getSysIdQuasistatic(SysIdRoutine routine, SysIdRoutine.Direction direction) {
     ...
     public static Command getSysIdFullCommand(SysIdRoutine routine, double quasistaticTimeout, double dynamicTimeout, double delay) {
        return getSysIdQuasistaticForward(routine).withTimeout(quasistaticTimeout)
