...
import frc.robot.FRC5010.motors.hardware.GenericRevPWMMotor;
...
public class Drive extends GenericMechanism {
    ...
    public static class Type {
        ...
        public static final String DIFF_DRIVE_2 = "DifferentialDriveTwoMotors";
    ...
    public Drive(VisionSystem visionSystem, GenericGyro gyro, String type, List<? extends DrivePorts> drivePorts,
        ...
        maxChassisVelocity = driveConstants.getkTeleDriveMaxSpeedMetersPerSecond();
        maxChassisRotation = driveConstants.getkTeleDriveMaxAngularSpeedRadiansPerSecond();
        mechVisual = new Mechanism2d(driveVisualH.getInteger(), driveVisualV.getInteger());
        SmartDashboard.putData("Drive Visual", mechVisual);
        initRealOrSim();
    }

        public Drive(VisionSystem visionSystem, GenericGyro gyro, String type,
            GenericDrivetrainConstants driveConstants, String driveTrainFolder) {
        super(Drive.class.getSimpleName());
        this.vision = visionSystem;
        this.gyro = gyro;
    ...
    protected void initRealOrSim() {
        switch (type) {
            ...
            case Type.DIFF_DRIVE_2: {
                initializeTwoMotorPWMDifferentialDrive();
                break;
            }
    ...
    private void initializeDifferentialDrive() {
        ...
        drivetrain = new DifferentialDrivetrain(template, motorPorts, gyro, vision, driveConstants, mechVisual);
    }

    private void initializeTwoMotorPWMDifferentialDrive() {
        GenericRevPWMMotor left = new GenericRevPWMMotor(motorPorts.get(0).getDrivePort());
        GenericEncoder leftEncoder = new WpiEncoder(new Encoder(4, 5));
        leftEncoder.setPositionConversion(motorPorts.get(0).getMotorConstants().getEncoderConversion());
        left.setMotorEncoder(leftEncoder);
        GenericRevPWMMotor right = new GenericRevPWMMotor(motorPorts.get(1).getDrivePort());
        right.invert(true);
        GenericEncoder rightEncoder = new WpiEncoder(new Encoder(6, 7));
        rightEncoder.setPositionConversion(motorPorts.get(1).getMotorConstants().getEncoderConversion());
        right.setMotorEncoder(rightEncoder);
      
        drivetrain = new DifferentialDrivetrain(left, right, gyro, vision, driveConstants, motorPorts, mechVisual);
    }
